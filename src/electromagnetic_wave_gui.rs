...
fn add_info_labels(mut commands: Commands, font: &Handle<Font>) {
    ...
    commands.spawn(generate_info_label(&font, "rotate around z: i", 80.0));
    commands.spawn(generate_info_label(&font, "rotate around y: o", 100.0));
    commands.spawn(generate_info_label(&font, "rotate around x: p", 120.0));
}
